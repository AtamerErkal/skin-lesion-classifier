...
if model is not None:
    ...
    <div class="upload-section">
        <h2 style="text-align: center; color: #2E4057; margin-bottom: 1rem;">
            📤 Upload or Capture Skin Lesion Image for Analysis
        </h2>
        <p style="text-align: center; color: #636e72; font-size: 1.1em;">
            📸 Choose a clear, well-lit image of the skin lesion<br>
            📷 Take a photo with your camera or upload an existing image<br>
    ...
    # Create two tabs for upload and camera
    ...
    with camera_tab:
        ...
        if camera_image is not None:
            st.info("📸 Photo captured! Analysis will begin automatically.")

    if uploaded_file is not None or camera_image is not None:
        try:
            # Determine which source to use
            image_source = camera_image if camera_image is not None else uploaded_file
            source_name = "Camera Photo" if camera_image is not None else uploaded_file.name
            ...
            st.info(f"📷 **Image Analysis Starting:** {source_name} ({image.size[0]}×{image.size[1]} pixels) | {len(image_bytes)/1024:.1f}KB")
            ...
            with col_img1:
                ...
                """, unsafe_allow_html=True)
                st.image(image, use_container_width=True, caption=f"Source: {source_name}")
            ...
            with col_img2:
                ...
                """, unsafe_allow_html=True)
                st.image(grad_cam_pil, use_container_width=True, caption="🔥 Grad-CAM: Red/yellow areas show AI focus")
            ...
            with tab4:
                ...
                if st.button("📥 Generate Downloadable Report", type="primary"):
                    ...
                    report_data = {
                        ...
                        "assessment": recommendation.replace('🚨', 'URGENT: ').replace('⚠️', 'WARNING: ').replace('🟢', 'BENIGN: ').replace('🔴', 'HIGH: ').replace('🟠', 'MONITOR: ').replace('🟡', 'BENIGN: ').replace('🔵', 'BENIGN: '),
                    ...
                    for pred in report_data['top_predictions']:
                        clean_pred = pred['Lesion Type'].replace('🎯', '').replace('🚨', '')
